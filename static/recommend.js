$(function() {
  ...
  $('.movie-button').on('click',function(){
    ...
    if (title=="") {
      ...
      load_details(my_api_key,title,userId);
    }
  });
});
...
function load_details(my_api_key,title,userId){
  $.ajax({
    type: 'GET',
    ...
    success: function(movie){
      if(movie.results.length<1){
        ...
        movie_recs(movie_title,movie_id,my_api_key,userId);
      }
    },
    error: function(){
...
function movie_recs(movie_title,movie_id,my_api_key,userId){
  $.ajax({
    type:'POST',
    url:"/similarity",
